...
class AbstractExperiment(object):
    ...
    def set_default_parameters(self):
        ...
        self.add_parameter('dictionary_capacity', int(5e5), comment='Dictionary capacity (per action)')  # 1e6 for MFEC, 5e5 for NEC
        ...
        self.add_parameter('learn_start', 50000, comment='Number of steps before starting training')  # 0 for MFEC, 50000 for NEC
        self.add_parameter('evaluate', False, comment='Evaluate only')
    ...
    def launch_simulation(self, sim_params, sim_id):
        ...
        if callable(sim_params["env"]):
            sim_params["env"] = sim_params["env"](sim_params["n_pretraining_tasks"], sim_params["n_training_test_tasks"], sim_params["pretraining_tasks_as_training_test_tasks"], sim_params["rng"])
